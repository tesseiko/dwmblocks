...
int logprint(const char *restrict format, ...);
...
void getcmd(const Block *block, char *output)
{
	...
	// read if it got nothing and the error was an interrupt could also just
	// read to a separate buffer and not move the data over if interrupted this
	// way will take longer trying to complete 1 thing but will get it done the
	// other way will move on to keep going with everything and the part that
	// failed to read will be wrong till its updated again either way you have
	// to save the data to a temp buffer because when it fails it writes
	// nothing and then then it gets displayed before this finishes

	char * s;
    int e;
    do {
        errno = 0;
		logprint("Reading commands output\n");
...
void statusloop()
{
	...
	logprint("Done calculating intervall\n");
	unsigned int i = 0;
    int interrupted = 0;
    const struct timespec sleeptime = {interval, 0};
    struct timespec tosleep = sleeptime;
	getcmds(-1);
	logprint("Done getting cmds\n");
	while(statusContinue)
	{
		//TODO
		fprintf(stderr,"Sleeping for %li sec\n", tosleep.tv_sec);
		...
		fprintf(stderr,"Getting cmds\n");
        // if not interrupted then do the calling and writing
        getcmds(i);
		fprintf(stderr,"Writing status\n");
        writestatus();
